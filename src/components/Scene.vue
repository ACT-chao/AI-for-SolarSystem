...
export default {
    ...
    async mounted(){
        ...
        // Store default camera position and controls settings
        this.defaultCameraPosition = camera.position.clone();
        controls.target.copy(this.defaultControlsTarget);
        controls.minDistance = this.defaultMinDistance;
        controls.maxDistance = this.defaultMaxDistance;

        this.planets = await this.createSolarSystem(scene);
        ...
        renderer.setAnimationLoop(() => {
            ...
            if(this.selectedPlanet) {
                this.selectedPlanet.children[0].getWorldPosition(controls.target);
            }
            controls.update();
    ...
    methods: {
        ...
        toggleView() {
            if(this.selectedPlanet) {
                ...
                if (sunPlanet) {
                    this.selectedPlanet = sunPlanet;
                    this.selectedPlanetCard = sunPlanet.userData;
                    let box = new THREE.Box3().setFromObject(sunPlanet.children[0].children[0]);
                    let diameter = Math.abs(box.max.x - box.min.x);
                    this.controls.minDistance = diameter * 1.25;
                    this.controls.maxDistance = diameter * 2.5;
                    sunPlanet.children[0].getWorldPosition(this.controls.target);
                }
            }
        },
    },
